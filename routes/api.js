...
//   User.find({ "emailId": query }, function (err, user) {
...
router.get("/emailId", function (request, response) {
  // get the emailid from the user
  let query = request.query.emailId;
  ...
  User.findOne({ emailId: query }, function (err, user) {
    response.status(200).json({id:user['_id'], username:user['username'], password:user['password'] });
    });
  }
});
